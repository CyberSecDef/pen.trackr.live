...
- **Measured for M4: an append costs roughly 20 ms on Windows** with `synchronous = FULL`, because every append is an fsync. That is the entire NFR-003 budget for command start delay, so the runner must append asynchronously off the hot path rather than before spawning. Worth knowing now rather than discovering it against a benchmark gate.

...
**Execution plan (17 September 2026; planning only):** Build the durable domain write path on the existing M2.3 ownership/registry layer and M2.4 schemas. Keep HTTP transport in M2.9, lifecycle transition policy in M2.6, and shared selection/switching in M2.7. The service must expose explicit admission inputs now so those later phases can supply authenticated context without bypassing revision and identity checks.

1. **Event contract and collection semantics.** Define versioned payloads for `scope.object-added` and `scope.excluded`; retain version-1 creation/update/state/switch readers and version-2 metadata readers without normalizing already hashed text. Specify separate compensating events for scope correction and removal where the existing event names do not describe them. Keep object ID and kind immutable on correction; classify inclusion versus exclusion explicitly. Reclassification keeps the same object ID, records one audited event, and cannot make the ID appear in both live sets. Route subsequent scope edits through scope events, even though the broad M2.4 patch schema can describe a complete scope snapshot; reserve a scope snapshot in `engagement.created` for initial state. Other metadata collection fields in `engagement.updated` replace the named collection; omitted fields preserve it, null clears nullable fields, and validated no-change patches append nothing. Document the exact event effects and replay defaults for version-1 genesis before writing them.
2. **Replay and project opening.** Add versioned engagement, scope, RoE/window, roster, and contact projections using `ProjectionRunner`. Replay starts from one verified creation event, upgrades version-1 metadata with M2.4's deterministic defaults, applies only recognized event type/version pairs, and checks identity, sequence, lifecycle snapshots, and roster mapping consistency. Historical roster IDs remain portable when a project moves to an installation with a different local operator ID; opening must not silently remap them. Provide an explicit audited roster reassignment command that binds the selected member to the current installation identity, while preserving the prior mapping in event history. Extend M2.3 inspection/open/list/mirror repair to derive current views from verified ledger history, including version-2 genesis and later events. An unknown version or invalid transition history blocks writes and reports `unsupported_project`/`invalid_history`; it must not be treated as an empty or current project.
3. **Serialized mutation service.** Admit an explicit registered/owned project and expected head revision under one per-project write queue; recheck the head after admission and reject stale writers. Generate envelope IDs, timestamps, operator attribution, and engagement ID in the core. Validate a domain command, append its typed event durably, catch projections up, and return committed event ID/head revision and view. Do not accept client-authored envelopes. Prepare an active-context check hook for M2.7/M2.9, without inventing an implicit selection in M2.5. The M2.6 transition service must reuse this path rather than append directly.
4. **Retry and failure recovery.** Use the caller's UUIDv7 idempotency key as `operation_id`; compute `command_hash` from a canonical validated command plus operator, method, route, and project identity. Lookup completed operations before stale-context/revision checks. A same-key/same-command retry returns the original result; changed use conflicts, and concurrent same-key attempts report `operation_in_progress`. Recover committed-event outcomes from the ledger even when the response or projection update failed. Persist successful no-op outcomes in the local registry without engagement events and exclude those local records from portable exports. If append commits but projection fails, report the committed event and repair state, block stale reads/writes, and finish catch-up on restart or explicit rebuild; never append the retry twice.
5. **Failure matrix and sign-off.** Test two writers racing one revision, retries before/after restart, changed-key reuse, no-op persistence, append failure, committed append with projection failure, unknown payload versions, damaged history, checkpoint/hash stability, and full projection rebuild against independently replayed views. Exercise two isolated projects and mirror repair after metadata changes. Run lint, typecheck, coverage, traceability, hygiene, build/package smoke, and three-OS CI before phase closeout. Do not claim FR-ENG requirements closed until the M2.9 query/mutation surface exists.
